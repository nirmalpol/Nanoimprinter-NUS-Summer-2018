...
class processMemory:
	...
	def setup(self):
		...
		#Setup PID
		self.pid_edge = pid_setup.pid_setup_edge(self.target_T_center,0,0,0)
		self.pid_center = pid_setup.pid_setup_center(self.target_T_edge,0,0,0)
		...
		#Set outputs to off
		self.pin_1.ChangeDutyCycle(0)
		self.pin_2.ChangeDutyCycle(0)
	...
	def run(self):
		...
		if self.T_mode == 'PWM':
			...
			raise #Error
		self.pin_1.ChangeDutyCycle(self.pwm_center)
		self.pin_2.ChangeDutyCycle(self.pwm_edge)
	...
	def loadData(self,seqcp):	
		...
		# [2] Timer '0'
		#		Only active and visible if [1] Endcon is (3) Timed
		# [3] Target T '80'
		#		Has value from 30 to 170, minimum step of 0.1 C
		# [4] Target P (over Kapton) '0'
		...
		# [10] (NOT CONTROLLABLE BY UI) Bottom of Kapton Vacuum '1'
		# [11] (BAD IDEA) Cool Air '1'

		self.setTarget(seqcp[3],seqcp[3],seqcp[4])
		self.transition()
		if seqcp[5] == 1:
			self.T_mode = 'PID'
			self.setTempPID(seqcp[7],seqcp[8],seqcp[9],seqcp[7],seqcp[8],seqcp[9])
		elif seqcp[5] == 0 or seqcp[5] == 2 or seqcp[5] == 3:
			self.T_mode = 'PWM'
			self.setTempPWM(seqcp[6])
		else:
			self.T_mode = None
	...
	#But only reset past phases (e.g. past PID control)
		self.pid_edge = pid_setup.pid_setup_edge(self.target_T_edge,0,0,0)
		self.pid_center = pid_setup.pid_setup_center(self.target_T_center,0,0,0)
